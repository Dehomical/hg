...
class SuperResolution:
    """基于OpenCV dnn_superres的超分辨率处理"""
    def __init__(self, model_path: str = 'EDSR_x4.pb', scale: int = 4):
        """
        :param model_path: 预训练模型路径
        :param scale: 放大倍数 (支持2/3/4倍)
        """
        # 初始化超分模型
        self.sr = cv2.dnn_superres.DnnSuperResImpl_create()
        self.sr.readModel(model_path)
        self.sr.setModel('edsr', scale)  # 可选 'edsr'/'espcn'/'fsrcnn'

    def enhance(self, img_path: str) -> np.ndarray:
        """
        执行超分辨率增强
        :return: 高分辨率图像 (H x W x 3)
        """
